...
`write`, `edit`, `bash`, `python`, `create_worktree`, `grep`, `lsp`, `web_search`,
...
Every native public-web capability name, plus `grep`, `python`, `schedule`,
`update_goal`, and `update_plan`, is reserved even when it is unavailable, so an
