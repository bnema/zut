...
const OnDemandSubagentsSystemAddendum = `Subagent tools are available when the user asks you to delegate or an active skill workflow requires delegation. Use them only in those cases. Mark delegated work required:true when the user's request or workflow requires its outcome before completion; failed required work must be retried and cannot be silently ignored. Do not delegate proactively or switch into an orchestrator role; otherwise perform the work yourself.`
