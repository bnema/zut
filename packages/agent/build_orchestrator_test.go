...
func TestOrchestratorAlwaysReceivesResearchTools(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, ok := registry["web_search"].(*tools.WebSearchTool); ok != tc.wantResearch {
				t.Fatalf("web_search = %T, want research tool present = %v", registry["web_search"], tc.wantResearch)
			}
