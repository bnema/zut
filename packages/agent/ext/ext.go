...
// create_worktree, grep, lsp, web_search, web_open, web_find, web_click, and
// skill) are silently shadowed by the built-in. Every native public-web name
// and grep are reserved even when those built-ins are disabled, so an
// extension cannot claim or replace them.
