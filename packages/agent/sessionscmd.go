...
// runSessionsCommand dispatches standalone session management commands
// before normal agent startup, so they do not require a provider or
// credentials.
func runSessionsCommand(rawArgs []string) (handled bool, err error) {
	...
	case "prune":
		...
		return true, runSessionsPrune(SessionsPath(), opts, os.Stdin, os.Stdout, os.Stderr, os.Stat)
	default:
		printSessionsHelp(os.Stderr)
...
func printSessionsHelp(out io.Writer) {
	fmt.Fprintln(out, `zut sessions - manage stored sessions

usage:
  zut sessions prune                         select missing directory groups
  zut sessions prune --older-than 30d        select sessions inactive for 30 days
  zut sessions prune --older-than 1mo --cwd PATH
  zut sessions prune --dry-run               list matches without deleting
  zut sessions prune --all [--yes]           select every match, optionally without prompting
...
func runSessionsPrune(sessionsRoot string, opts sessionsPruneOptions, in io.Reader, out, errOut io.Writer, stat statPathFunc) error {
	groups, scanIssues := core.ScanStoredSessionGroups(sessionsRoot)
	...
	for _, idx := range selected {
		...
		if opts.olderThan == nil {
			// Recheck just before deleting: a directory that reappeared
			// (remount, restore) must not lose its sessions.
			_, err := stat(group.CWD)
			if err == nil {
