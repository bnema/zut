...
func TestFloatingPaneComposeCentersAndDimsLiveBackground(t *testing.T) {
	...
	frame, cursorRow, cursorCol := pane.Compose(Dark, "settings", []string{"live transcript", "draft input"}, []string{"settings body"}, 100, 30, -1, 0)
...
func TestFloatingPaneUsesBottomDrawerBelowEightyColumns(t *testing.T) {
	var pane FloatingPane
	frame, _, _ := pane.Compose(Dark, "settings", nil, []string{"settings body"}, 79, 24, -1, 0)
	...
	if got := stripANSI(frame.Lines[0]); strings.Trim(got, "─") != "" {
		t.Fatalf("drawer top border = %q, want rule only", got)
	}
}

func TestFloatingPaneRecomputesGeometryOnResize(t *testing.T) {
	var pane FloatingPane
	wide, _, _ := pane.Compose(Dark, "settings", nil, []string{"setting"}, 100, 30, -1, 0)
	narrow, _, _ := pane.Compose(Dark, "settings", nil, []string{"setting"}, 79, 24, -1, 0)
...
func TestFloatingPaneClampsCursorToContentAndHandlesTinyTerminals(t *testing.T) {
	var pane FloatingPane
	frame, _, col := pane.Compose(Dark, "search", nil, []string{"query"}, 100, 20, 0, 1000)
	if want := frame.Rect.contentX() + frame.Rect.ContentWidth() - 1; col != want {
		t.Fatalf("cursor col = %d, want content edge %d", col, want)
	}

	tiny, row, tinyCol := pane.Compose(Dark, "search", nil, []string{"x"}, 2, 2, 0, 10)
...
func TestFloatingPaneKeepsFocusedContentVisibleAndResetsForOtherView(t *testing.T) {
	...
	var pane FloatingPane
	frame, cursorRow, _ := pane.Compose(Dark, "settings", nil, content, 100, 12, 25, 0)
	...
	other, _, _ := pane.Compose(Dark, "subagents", nil, content, 100, 12, -1, 0)
...
func TestRendererDrawFloatingPaintsBackgroundThenPaneAtAbsolutePosition(t *testing.T) {
	...
	r.Resize(100, 20)
	var pane FloatingPane
	frame, _, _ := pane.Compose(Dark, "settings", []string{"background"}, []string{"foreground"}, 100, 20, -1, 0)
