...
- `ast`: search source code structurally with an [ast-grep](https://ast-grep.github.io/) pattern. Zut starts an installed `sg` executable directly without a shell, bounds its JSON-lines output, and follows the active filesystem read scope. Install ast-grep separately; the tool returns a clear error when `sg` is unavailable.
